    ...
    stages { 
        ...
        stage('jqassistant Analysis'){
         ...
         stage('Static Analysis'){
                 steps {
                     script {
                         sh "bat /sonar-scanner/sonar-scanner-3.2.0.1227-windows/bin/sonar-scanner -Dsonar.login={token} -Dsonar.projectBaseDir=. -Dsonar.projectKey='${params.sonarProjectKey}' -Dsonar.sources=. -Dsonar.java.binaries=. -Dsonar.branch.name='${params.branch}' -DtoolStackRulesPath=/var/jenkins_home/Rules/tool_stack_rules.xml -DserviceComplianceRulesPath=/var/jenkins_home/Rules/service_compliance_rules.xml -DanchoreJsonParser=/var/jenkins_home/Rules/anchoreengine-api-response-vulnerabilities-1.json -DconfigurationRulesPath=/var/jenkins_home/Rules/config-compliance-rules.xml -DcicdComplianceRulesPath=/var/jenkins_home/Rules/ci-cd_compliance_rules.xml -DarchitectureComplianceReportPath=/var/jenkins_home/Rules/jqassistant/reports/jqassistant-report.xml"
